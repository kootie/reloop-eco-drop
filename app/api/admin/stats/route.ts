...
export async function GET(request: NextRequest) {
  try {
    ...
    const pendingDrops = allDrops?.filter(drop => drop.status === 'pending') || []
    const approvedDrops = allDrops?.filter(drop => drop.status === 'approved') || []
    const rejectedDrops = allDrops?.filter(drop => drop.status === 'rejected') || []

    // Calculate real statistics from database data
    const totalTokensAwarded = approvedDrops.reduce((sum, drop) => 
      sum + (parseFloat(drop.actual_reward_ada) || 0), 0
    )
    ...
    // Bin statistics  
    const activeBins = allBins?.filter(bin => bin.status === 'active') || []
    ...
    const recentDrops = allDrops?.filter(drop => 
      drop.created_at && new Date(drop.created_at) > sevenDaysAgo
    ).length || 0
    ...
    const monthlyDrops = allDrops?.filter(drop => {
      if (!drop.created_at) return false
      const dropDate = new Date(drop.created_at)
      return dropDate.getMonth() === currentMonth && 
             dropDate.getFullYear() === currentYear
    }).length || 0
    
    const monthlyTokens = approvedDrops.filter(drop => {
      if (!drop.created_at) return false
      const dropDate = new Date(drop.created_at)
      return dropDate.getMonth() === currentMonth && 
             dropDate.getFullYear() === currentYear
    }).reduce((sum, drop) => sum + (parseFloat(drop.actual_reward_ada) || 0), 0)
    ...
    return NextResponse.json({
      ...
      stats: {
        ...
        bins: {
          ...
          maintenance: allBins?.filter(bin => bin.status === 'maintenance').length || 0,
          inactive: allBins?.filter(bin => bin.status === 'inactive').length || 0
        },
        ...
        users: {
          ...
          activeThisMonth: new Set(allDrops
            ?.filter(drop => {
              if (!drop.created_at) return false
              const dropDate = new Date(drop.created_at)
              return dropDate.getMonth() === currentMonth && 
        ...
        tokens: {
          ...
          averagePerDrop: approvedDrops.length > 0 ? 
            Math.round((totalTokensAwarded / approvedDrops.length) * 1000000) / 1000000 : 0
        }
      }
    })
  } catch (error) {

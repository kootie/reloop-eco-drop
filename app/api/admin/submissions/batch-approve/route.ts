...
import { supabase, DropService, UserService } from '@/lib/supabase'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (token.startsWith('admin_')) {
      ...
      console.log('Admin token detected for batch approval:', token.substring(0, 20) + '...')
      userId = 'admin'
    } else {
    ...
    for (const submission of submissions) {
      if (!submission.dropId || !submission.userId || !submission.actualRewardAda) {
        return NextResponse.json(
    ...
    const userAddresses = new Map(users?.map(u => [u.user_id, u.cardano_address]) || [])

    // Check for users without Cardano addresses
    const usersWithoutAddresses = userIds.filter(id => !userAddresses.get(id))
    if (usersWithoutAddresses.length > 0) {
      console.log(`Warning: ${usersWithoutAddresses.length} users without connected Eternl wallets`)
    }
    ...
    try {
      const payments = submissions
        ...
        .map(submission => ({
          address: userAddresses.get(submission.userId)!,
    ...
    try {
      ...
      for (const submission of submissions) {
        if (processedSubmissions.includes(submission.dropId) && userAddresses.get(submission.userId)) {
          await UserService.processPayment(submission.userId, submission.actualRewardAda)
        }
      }

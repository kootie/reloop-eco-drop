...
export async function POST(request: NextRequest) {
  try {
    ...
    const requiredFields = ['location_name', 'address', 'latitude', 'longitude', 'qr_code']
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (updateData.longitude) dbUpdateData.longitude = parseFloat(updateData.longitude)
    if (updateData.capacity_kg) dbUpdateData.capacity_kg = parseFloat(updateData.capacity_kg)
    if (updateData.current_fill_kg) dbUpdateData.current_fill_kg = parseFloat(updateData.current_fill_kg)
    if (updateData.reward_rate_per_kg) dbUpdateData.reward_rate_per_kg = parseFloat(updateData.reward_rate_per_kg)
    if (updateData.status) dbUpdateData.status = updateData.status
    if (updateData.retailer_name) dbUpdateData.retailer_name = updateData.retailer_name
    if (updateData.contact_phone) dbUpdateData.contact_phone = updateData.contact_phone
    if (updateData.operating_hours) dbUpdateData.operating_hours = updateData.operating_hours

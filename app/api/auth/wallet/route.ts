...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!cardanoAddress || typeof cardanoAddress !== 'string') {
      return NextResponse.json(
        { success: false, error: 'Cardano address is required and must be a string' },
        { status: 400 }
      )
    }
    ...
    try {
      ...
      if (pendingError) {
        ...
        console.log(`Found ${pendingDrops.length} pending payments for user ${decoded.userId}`)
        
        // Send payments for all pending drops
        const payments = pendingDrops.map(drop => ({
          address: cardanoAddress,
          amount: drop.actual_reward_ada || drop.estimated_reward_ada,
        ...
        if (payments.length > 0) {
          ...
          // Update user balance
          const totalPendingAmount = pendingDrops.reduce((sum, drop) => 
            sum + (drop.actual_reward_ada || drop.estimated_reward_ada), 0
          )

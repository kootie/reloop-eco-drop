...
export async function POST(request: NextRequest) {
  try {
    ...
    const estimatedTokens =
      riskLevelRates[itemType as keyof typeof riskLevelRates] || 0.5;
    ...
    const { data: drop, error } = await supabase
      ...
      .insert({
        ...
        bin_id: bin.id,
        device_type_id:
          deviceType?.id || "00000000-0000-0000-0000-000000000000", // Default UUID if device type not found
...
export async function GET(request: NextRequest) {
  try {
    ...
    const formattedDrops =
      userDrops?.map((drop) => ({
        id: drop.drop_id,
        binId: "Unknown", // Will be fetched separately if needed
        itemType: drop.device_type_id,

...
export async function GET(
	...
	try {
		...
		if (error || !bin) {
			console.warn('[QR API] Bin not found after normalized or-query. Error:', error?.message)
		}
		...
		const fillPercentage = typeof bin.fill_percentage === 'number'
			? Math.round(bin.fill_percentage)
			: bin.capacity_kg > 0
				? Math.round(((bin.current_weight_kg || 0) / bin.capacity_kg) * 100)
				: 0
		...
		const binInfo = {
			...
			capacity: {
				...
				fillPercentage,
				available: (bin.capacity_kg as number) - ((bin.current_weight_kg as number) || 0)
			},
			stats: {
				totalDrops: (bin.total_drops as number) || 0,
				lastMaintenance: (bin.updated_at as string) || (bin.created_at as string),
				rewardRate: 0.5
			},
			riskLevels: {
				level1: {

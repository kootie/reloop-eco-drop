...
async function testPaymentSetup() {
  ...
  if (!blockfrostId) {
    ...
    console.log(`✅ BLOCKFROST_PROJECT_ID: ${blockfrostId.substring(0, 10)}...`)
  }
  ...
  if (!walletSeed) {
    ...
    if (words.length === 24) {
      ...
      console.log(`❌ BACKEND_WALLET_SEED: ${words.length} words (should be 24)`)
    }
  }
  ...
  if (blockfrostId) {
    if (blockfrostId.startsWith('preprod')) {
      ...
      console.log('❓ Unknown network - Should start with "preprod" or "mainnet"')
    }
  } else {

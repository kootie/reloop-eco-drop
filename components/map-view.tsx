...
export default function MapView({ user, onBack, onSelectBin }: MapViewProps) {
  ...
  const getDefaultBins = useCallback(
    (): BinLocation[] => [
      {
        id: "zugdidi_001",
        ...
        setBins(
          data.bins.map(
            (bin: {
              ...
              status:
                bin.is_active && bin.is_operational
                  ? "active"
                  : bin.status || "inactive",
          ...
          <CardContent>
            <div className="bg-green-100 rounded-lg p-8 text-center border-2 border-dashed border-green-300">
              ...
              <p className="text-green-600 text-sm">
                {t.map.showingBins.replace("{count}", bins.length.toString())}
              </p>
              {userLocation && (
                <p className="text-green-500 text-xs mt-2">
                  {t.map.yourLocation}: {userLocation.lat.toFixed(4)},{" "}
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <Badge
                          ...
                          className={
                            bin.status === "active" ? "bg-green-600" : ""
                          }
                        >
                          {bin.status}
                        </Badge>
          ...
          <Card className="mt-6 border-green-500 bg-green-50">
            ...
            <CardContent>
              <div className="flex gap-3">
                <Button
                  onClick={handleStartDropFromSelection}
                  className="bg-green-600 hover:bg-green-700"
                >

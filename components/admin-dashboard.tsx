...
export default function AdminDashboard({
  ...
  const handleDropReview = async (
    dropId: string,
    action: "approve" | "reject",
  ) => {
    if (
      action === "approve" &&
      (!reviewTokens || parseFloat(reviewTokens) < 0)
    ) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
          ...
          <Card className="border-purple-200">
            <CardContent className="p-6 text-center">
              ...
              <div className="text-2xl font-bold text-purple-800">
                {stats.totalTokensAwarded.toFixed(1)}
              </div>
        ...
        {activeTab === "verification" && <AdminVerification admin={admin} />}
              ...
              <CardContent>
                <div className="flex gap-4">
                  <Button
                    ...
                    className={
                      statusFilter === "pending" ? "bg-yellow-600" : ""
                    }
                  >
                  ...
                  <Button
                    ...
                    className={
                      statusFilter === "approved" ? "bg-green-600" : ""
                    }
                  >
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-3 mb-2">
                              ...
                              <Badge variant="outline">
                                {
                                  riskLevelNames[
                                    drop.itemType as keyof typeof riskLevelNames
                                  ]
                                }
                              </Badge>
                            </div>
                            ...
                            <div className="grid gap-2 md:grid-cols-2 text-sm text-gray-600">
                              ...
                              <div>
                                ...
                                {drop.estimatedTokens} ADA
                              </div>
                              <div>
                                <strong>Submitted:</strong>{" "}
                                {formatDate(drop.submittedAt)}
                              </div>
                              {drop.approvedAt && (
                                <div>
                                  <strong>Reviewed:</strong>{" "}
                                  {formatDate(drop.approvedAt)} by{" "}
                                  {drop.approvedBy || "Unknown"}
                                </div>
                              )}
                            </div>
      ...
      {selectedDrop && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <Card className="w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <CardHeader>
              <CardTitle className="text-blue-800">
                Review Drop #{selectedDrop.id}
              </CardTitle>
              <CardDescription>
                Submitted by {selectedDrop.userEmail} on{" "}
                {formatDate(selectedDrop.submittedAt)}
              </CardDescription>
            </CardHeader>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  ...
                  <p className="text-gray-600">
                    {
                      riskLevelNames[
                        selectedDrop.itemType as keyof typeof riskLevelNames
                      ]
                    }
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-gray-600">
                    {selectedDrop.estimatedTokens} ADA
                  </p>
                </div>
                <div>
                  <Label className="text-sm font-semibold text-gray-700">
              ...
              {selectedDrop.status === "pending" && (
                <div className="space-y-4 border-t pt-4">
                  ...
                  <div className="flex gap-3 pt-4">
                    <Button
                      onClick={() =>
                        handleDropReview(selectedDrop.id, "approve")
                      }
                    ...
                    <Button
                      onClick={() =>
                        handleDropReview(selectedDrop.id, "reject")
                      }
                      disabled={isSubmitting}
                      variant="destructive"
                    >
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="space-y-2 text-sm text-gray-600">
                      <div>
                        ...
                        {selectedDrop.status.charAt(0).toUpperCase() +
                          selectedDrop.status.slice(1)}
                      </div>
                      <div>
                        <strong>Reviewed by:</strong>{" "}
                        {selectedDrop.approvedBy || "Unknown"}
                      </div>
                      <div>
                        <strong>Review date:</strong>{" "}
                        {selectedDrop.approvedAt &&
                          formatDate(selectedDrop.approvedAt)}
                      </div>
                      {selectedDrop.actualTokens !== null && (
                        <div>
                          <strong>Tokens awarded:</strong>{" "}
                          {selectedDrop.actualTokens} ADA
                        </div>
                      )}
                      {selectedDrop.notes && (
                        <div>

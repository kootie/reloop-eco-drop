...
type Database = {
  public: {
    Tables: {
      ...
      device_types: {
        Row: DeviceType;
        Insert: Omit<DeviceType, 'id'> & { id?: string };
        Update: Partial<Omit<DeviceType, 'id'>>;
      };
      bins: {
        Row: Bin;
        Insert: Omit<Bin, 'id'> & { id?: string };
        Update: Partial<Omit<Bin, 'id'>>;
      };
    };
  };
};

// Supabase configuration - only initialize when environment variables are available
...
export class UserService {
  ...
  static async updateBalanceAndStats(
    ...
    const updates = {
      pending_rewards_ada:
        user.pending_rewards_ada + (drop_approved ? reward_ada : 0),
      total_earned_ada:
        user.total_earned_ada + (drop_approved ? reward_ada : 0),
      total_drops: user.total_drops + 1,
      successful_drops: drop_approved
        ? user.successful_drops + 1
        : user.successful_drops,
      rejected_drops: drop_approved
        ? user.rejected_drops
        : user.rejected_drops + 1,
      updated_at: new Date().toISOString(),
    };
  ...
  static async getUsersWithPendingRewards(
    min_amount_ada: number = 1.0,
  ): Promise<User[]> {
    const supabase = getSupabaseClient();
    const { data, error } = await supabase
      .from("users")
...
export class DropService {
  ...
  static async getPendingDrops(limit: number = 50): Promise<
    Array<{
      drop_id: string;
      user_id: string;
      status: string;
  ...
  static async approve(
    ...
    await UserService.updateBalanceAndStats(
      data.user_id,
      actual_reward_ada,
      true,
    );
  ...
  static async getApprovedDropsForPayment(limit: number = 100): Promise<
    Array<{
      drop_id: string;
      user_id: string;
      actual_reward_ada: number;
  ...
  static async updateAfterDrop(
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        total_rewards_distributed_ada:
          bin.total_rewards_distributed_ada + reward_ada,

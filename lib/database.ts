...
export class DropService {
  ...
  static async markAsPaid(dropIds: string[], txHash: string, batchId: string) {
    try {
      ...
      if (error) throw error;
      console.log(
        `✅ Marked ${dropIds.length} drops as paid with TX: ${txHash}`,
      );
    } catch (error) {
...
export class UserService {
  ...
  static async processPayment(userId: string, amountADA: number) {
    try {
      ...
      const currentTotal = (user?.total_earned_ada || 0) as number
      const currentPending = (user?.pending_balance_ada || 0) as number
  ...
  static async getUserStats(userId: string) {
    try {
      ...
      const stats = {
        ...
        approvedDrops:
          drops?.filter((d: any) => d.status === "approved").length || 0,
        pendingDrops:
          drops?.filter((d: any) => d.status === "pending").length || 0,
        rejectedDrops:
          drops?.filter((d: any) => d.status === "rejected").length || 0,
        totalEarned:
          drops?.reduce((sum, d: any) => sum + ((d.actual_reward_ada || 0) as number), 0) || 0,
      };

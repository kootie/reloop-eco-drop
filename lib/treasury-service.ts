...
export class TreasuryService {
  ...
  async fundTreasury(
    ...
    try {
      if (!this.lucid || !this.treasuryAddress) {
        ...
        // Update database
        await this.recordTreasuryFunding(
          amountADA,
          adminWalletAddress,
          adminId,
          txHash,
        ...
        console.log(
          `✅ [DEMO] Funded treasury with ${amountADA} ADA from ${adminWalletAddress}`,
        );
      ...
      const tx = this.lucid
        .newTx()
        .payToAddress(this.treasuryAddress, {
  ...
  async processPayout(
    ...
    try {
      ...
      if (!this.lucid || !this.treasuryAddress) {
        ...
        // Update database
        await this.recordTreasuryPayout(
          dropId,
          recipientAddress,
          amountADA,
          userId,
        ...
        console.log(
          `✅ [DEMO] Processed payout: ${amountADA} ADA to ${recipientAddress}`,
        );
        return { success: true, txHash };
      }
      ...
      console.log(
        `✅ Processed treasury payout: ${amountADA} ADA to ${recipientAddress}, TX: ${txHash}`,
      );
      return { success: true, txHash };
    } catch (error) {
      const errorMessage =
  ...
  async getTreasuryStats(): Promise<{
    ...
    try {
      ...
      const totalFunded =
        fundingStats?.reduce((sum, tx) => sum + (tx.amount_ada || 0), 0) || 0;
      const totalPaidOut =
        payoutStats?.reduce((sum, tx) => sum + (tx.amount_ada || 0), 0) || 0;
      const pendingPayouts =
        pendingDrops?.reduce(
          (sum, drop) => sum + (drop.actual_tokens || 0),
          0,
        ) || 0;
  ...
  private async verifyAdminWallet(

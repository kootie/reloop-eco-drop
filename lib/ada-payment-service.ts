...
export class ADAPaymentService {
  ...
  private async initializeLucid(): Promise<void> {
    try {
      ...
      if (balance.ada < 10) {
        console.warn(
          `⚠️ Low balance: ${balance.ada} ADA. Consider adding more testnet ADA`,
        );
      }
    } catch (error) {
  ...
  async sendADA(
    ...
    try {
      ...
      if (!this.lucid) {
        ...
        console.log(
          `✅ [DEMO] Sent ${amountADA} ADA to ${recipientAddress}, TX: ${txHash}`,
        );
      ...
      console.log(
        `✅ Sent ${amountADA} ADA to ${recipientAddress} on testnet, TX: ${txHash}`,
      );
      console.log(`📝 Memo: ${memo || "No memo"}`);
  ...
  async sendBatchADA(
    ...
    try {
      ...
      if (!this.lucid) {
        ...
        console.log(
          `✅ [DEMO] Batch payment: ${totalADA} ADA to ${payments.length} recipients, TX: ${txHash}`,
        );
        return {
          txHash,
      ...
      for (const payment of payments) {
        ...
        if (lovelaceAmount < 1_000_000) {
          console.warn(
            `Skipping payment to ${payment.address}: amount ${payment.amount} ADA is below minimum`,
          );
          continue;
        }
      ...
      console.log(
        `✅ Batch payment completed on testnet: ${totalADA} ADA to ${payments.length} recipients, TX: ${txHash}`,
      );
  ...
  async processApprovedDrops(maxDrops: number = 50): Promise<{
    ...
    try {
      ...
      const approvedDrops =
        await DropService.getApprovedDropsForPayment(maxDrops);
      ...
      const totalRequired = approvedDrops.reduce(
        (sum, drop: any) => sum + ((drop.actual_reward_ada || 0) as number),
        0,
      );

      if (backendBalance.ada < totalRequired + 5) {
        // Keep 5 ADA buffer for fees
        throw new Error(
          `Insufficient backend balance: ${backendBalance.ada} ADA available, ${totalRequired} ADA required`,
        );
      }
      ...
      for (const drop of approvedDrops) {
        const key = (drop as any).cardano_address as string;
        if (userPayments.has(key)) {
          const existing = userPayments.get(key)!;
          existing.amount += (drop as any).actual_reward_ada || 0;
          existing.dropIds.push((drop as any).drop_id);
        } else {
          userPayments.set(key, {
            address: key,
            amount: (drop as any).actual_reward_ada || 0,
            dropIds: [(drop as any).drop_id],
            userId: (drop as any).user_identifier || '',
          });
        }
      }
      ...
      const allDropIds = approvedDrops.map((drop: any) => drop.drop_id as string);
      ...
      console.log(
        `✅ Processed ${approvedDrops.length} drops, paid ${result.totalAmount} ADA to ${result.recipientCount} users`,
      );
  ...
  async sendIndividualReward(
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Unknown error";
      console.error(
        `❌ Failed to send individual reward for drop ${dropId}:`,
        error,
      );
